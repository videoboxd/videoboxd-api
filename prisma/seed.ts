...
async function main() {
  ...
  for (const videoData of videos) {
    ...
    if (!platform || !user) {
      console.info("Platform or user not found");
      continue;
    }
    ...
    if (!category) {
      console.info(`Kategori dengan slug ${categorySlug} tidak ditemukan.`);
      continue;
    }
  ...
  for (const reviewData of reviews) {
    ...
    if (existingReview) {
      ...
      console.info(
        `⭐ Review Updated: (${updatedReview.rating}) ${updatedReview.text}`
      );
    } else {
      // Buat review baru
      ...
      console.info(
        `⭐ Review Created: (${newReview.rating}) ${newReview.text}`
      );
    }
  }
  // Comments on Videos
  ...
  for (const playlistData of playlists) {
    ...
    if (!user) {
      console.info(`User tidak ditemukan untuk playlist ${playlistData.title}`);
      continue;
    }
